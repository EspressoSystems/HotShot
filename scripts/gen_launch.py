...
def get_crates() -> list[str]:
    return subprocess.run(["cargo-workspaces", "workspaces", "list"],
                          stdout=subprocess.PIPE).stdout.decode().split()


def get_test_list(crate: str) -> list[dict]:
    lines_json: Final[str] = subprocess.run(
        ["just", "list_tests_json", crate], stdout=subprocess.PIPE).stdout.decode()
    return get_launch_json_tests(lines_json, crate)


def get_example_list(crate: str) -> list[dict]:
    lines: Final[str] = subprocess.run(
        ["just", "list_examples", crate], stdout=subprocess.PIPE).stdout.decode()
