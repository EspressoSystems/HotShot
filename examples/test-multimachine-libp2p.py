...
def gen_invocation(
    ...
    fmt_cmd = [
        f'cargo run --example=multi-machine --release -- ' \
...
if __name__ == "__main__":
    ...
    for i in range(0, len(bootstrap_addrs)):
        ...
        bootstrap_cmd = gen_invocation(
            ...
            seed=SEED,
            node_id=i*(NUM_NODES_PER_BOOTSTRAP + 1)
        );
        bootstrap_cmds.append(bootstrap_cmd);

        for j in range(0, len(regulars_list)):
            regular_cmd = gen_invocation(
                node_type=NodeType.REGULAR,
                num_nodes=TOTAL_NUM_NODES,
                to_connect_addrs=bootstrap_addrs,
                num_rounds=NUM_ROUNDS,
                bound_addr=regulars_list[j],
                node_id=i * (NUM_NODES_PER_BOOTSTRAP + 1) + j + 1,
                seed=SEED
            );
