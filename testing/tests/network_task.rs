...
async fn test_network_task() {
    ...
    let quorum_exchange = api.inner.exchanges.quorum_exchange();
    let committee_exchange = api.inner.exchanges.committee_exchange().clone();
    let view_sync_excahnge = api.inner.exchanges.view_sync_exchange();
    ...
    let build_fn = |task_runner, event_stream| {
        add_network_message_task(task_runner, event_stream, quorum_exchange);
        add_network_message_task(task_runner, event_stream, committee_exchange);
        add_network_message_task(task_runner, event_stream, view_sync_excahnge);
