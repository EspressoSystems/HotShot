...
pub async fn spin_up_swarms<S: Debug + Default>(
    ...
    for i in 0..num_bootstrap {
        ...
        // port
        // let addr = Multiaddr::from_str(&format!("/ip4/127.0.0.1/udp/{}/quic", start_port + i)).unwrap();

        let addr = Multiaddr::from_str("/ip4/127.0.0.1/udp/0/quic").unwrap();
        config
            .identity(identity)
