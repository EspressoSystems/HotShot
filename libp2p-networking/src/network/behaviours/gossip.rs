use std::{collections::{VecDeque, HashSet}, task::Poll};
    ...
    fn inject_event(&mut self, event: GossipsubEvent) {
        match event {
            GossipsubEvent::Message { message, .. } => {
                ...
                self.out_event_queue.push(GossipEvent::GossipMsg(message.data, message.topic))
...
impl GossipBehaviour {
    ...
    pub fn subscribe_gossip(&mut self, t: &str) {
        if self.subscribed_topics.contains(t) {
            warn!("tried to subscribe to already subscribed topic {:?}. Noop.", t);
        } else {
