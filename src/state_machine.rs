...
#![allow(
    ...
    clippy::cast_lossless, // TODO(#170): This file will be gone soon
        ...
        match self {
            ...
            SequentialLeader::Start => {
                ...
                let fut = async move {
                    ...
                    let signature = pl.sign_vote(&the_hash, Stage::Prepare, current_view);
                ...
                Poll::Ready(x) => {
                    ...
                    let fut = async move {
                        ...
                        let votes: Vec<_> = vote_queue
                            ...
                            .collect();
                        // Generate the QC
                        let signatures = generate_qc(
                            votes.iter().map(|(x, z)| (*x, z)),
                            &pl.inner.cluster_public_keys,
                        )?;
                        ...
                        *pqc = Some(qc.clone());
                        ...
                        let signature =
                            pl.sign_vote(&new_leaf_hash, Stage::PreCommit, current_view);
            ...
            SequentialLeader::Precommit(fut) => {
                if let Poll::Ready(x) = Future::poll(fut.0.as_mut(), cx) {
                    ...
                    let fut = async move {
                        ...
                        let signature = pl.sign_vote(&new_leaf_hash, Stage::Commit, current_view);
        ...
        match self {
            ...
            BeforePrepare(prepare) => {
                ...
                let fut = async move {
                    ...
                    if is_safe_node && state.validate_block(&leaf.item) {
                        let signature = pl.sign_vote(&leaf_hash, Stage::Prepare, current_view);
                        let vote = PrepareVote(Vote {
                            signature,
                            id: pl.inner.public_key.nonce,
                            leaf_hash,
            ...
            BeforePreCommit { leaf, message } => {
                ...
                let fut = async move {
                    ...
                    if !(prepare_qc.verify(
                        &pl.inner.cluster_public_keys,
                        pl.inner.config.threshold as u64,
                        current_view,
                        Stage::Prepare,
                    ...
                    let signature = pl.sign_vote(&leaf_hash, Stage::PreCommit, current_view);
            ...
            BeforeCommit { leaf, message } => {
                ...
                let fut = async move {
                    ...
                    *locked_qc = Some(pc_qc);
                    ...
                    let signature = pl.sign_vote(&leaf_hash, Stage::Commit, current_view);
