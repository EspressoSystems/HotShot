...
use crate::{HotStuff, HotStuffConfig, PubKey, H_256};
use blake3::Hasher;
...
pub async fn try_hotstuff(
    ...
    let genesis = DEntryBlock {
        previous_block: (*Hasher::new().finalize().as_bytes()).into(),
        transactions: vec![],
    };
...
mod test {
    ...
    async fn hotstuff_dentry_demo() {
        ...
        let hotstuffs: Vec<(HotStuff<DEntryBlock, H_256>, PubKey, u16, WNetwork<_>)> =
        ...
        join_all(hotstuffs.iter().map(|(h, _, _, _)| h.run_round(1, None))).await;
        println!(
            "Current states: {:?}",
            join_all(hotstuffs.iter().map(|(h, _, _, _)| h.get_state())).await
        );
        ...
        join_all(hotstuffs.iter().map(|(h, _, _, _)| h.run_round(2, None))).await;
        println!(
            "Current states: {:?}",
            join_all(hotstuffs.iter().map(|(h, _, _, _)| h.get_state())).await
        );
        ...
        join_all(hotstuffs.iter().map(|(h, _, _, _)| h.run_round(3, None))).await;
        println!(
            "Current states: {:?}",
            join_all(hotstuffs.iter().map(|(h, _, _, _)| h.get_state())).await
        );
