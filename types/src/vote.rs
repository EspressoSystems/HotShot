...
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq)]
#[serde(bound(deserialize = ""))]
pub struct ViewSyncVote<TYPES: NodeType> {
    // TODO ED Maybe we just define a type as this tuple?
    ...
    pub vote_token: TYPES::VoteTokenType,
    /// The vote data this vote is signed over
    pub vote_data: VoteData<ViewSyncData>,
}

#[derive(Serialize, Deserialize, Clone, Debug, PartialEq)]
#[serde(bound(deserialize = ""))]
pub struct ViewSyncData {
    pub relay: EncodedPublicKey,
    pub round: u64,
}

impl Committable for ViewSyncData {
    fn commit(&self) -> Commitment<Self> {
        let mut builder = commit::RawCommitmentBuilder::new("Quorum Certificate Commitment");

        builder
            .var_size_field(&format!("Relay public key"), &self.relay.0)
            .u64(self.round)
            .finalize()
    }
}
...
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq)]
#[serde(bound(deserialize = ""))]
pub enum ViewSyncStage<TYPES: NodeType> {
    PreCommit(ViewSyncVote<TYPES>),
    Commit(ViewSyncVote<TYPES>),
    Finalize(ViewSyncVote<TYPES>),
}
