...
impl<TYPES: NodeType, I: NodeImplementation<TYPES>, V: Versions> SystemContext<TYPES, I, V> {
    ...
    pub async fn new_from_channels(
        ...
        let anchored_epoch = if config.epoch_height == 0 {
            TYPES::EpochTime::new(0)
        } else if anchored_leaf.height() % config.epoch_height == 0 {
            TYPES::EpochTime::new(anchored_leaf.height() / config.epoch_height)
        } else {
            TYPES::EpochTime::new(anchored_leaf.height() / config.epoch_height + 1)
        };
    ...
    pub async fn start_consensus(&self) {
            ...
            if let Some(validated_state) = consensus.validated_state_map().get(&self.start_view) {
                ...
                self.internal_event_stream
                    ...
                    .broadcast_direct(Arc::new(HotShotEvent::ValidatedStateUpdated(
                        TYPES::ViewTime::new(*self.start_view),
...

        {
            ...
            if self.anchored_leaf.view_number() == TYPES::ViewTime::genesis() {
...
pub struct HotShotInitializer<TYPES: NodeType> {
    ...
    undecided_state: BTreeMap<TYPES::ViewTime, View<TYPES>>,
    /// Proposals we have sent out to provide to others for catchup
    saved_proposals: BTreeMap<TYPES::ViewTime, Proposal<TYPES, QuorumProposal<TYPES>>>,
}
...
impl<TYPES: NodeType> HotShotInitializer<TYPES> {
    ...
    pub async fn from_genesis<V: Versions>(
        ...
        Ok(Self {
            ...
            start_view: TYPES::ViewTime::new(0),
            actioned_view: TYPES::ViewTime::new(0),
