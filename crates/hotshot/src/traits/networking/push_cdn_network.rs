...
impl<T: SignatureKey> SignatureScheme for WrappedSignatureKey<T> {
    ...
    fn sign(private_key: &Self::PrivateKey, message: &[u8]) -> anyhow::Result<Vec<u8>> {
        let signature = T::sign_arbitrary(private_key, message)?;
    ...
    fn verify(public_key: &Self::PublicKey, message: &[u8], signature: &[u8]) -> bool {
        ...
        public_key.0.validate_arbitrary(&signature, message)
    }
}
...
impl<TYPES: NodeType> PushCdnNetwork<TYPES> {
    ...
    pub async fn new(
        ...
        let mut computed_topics: Vec<PushCdnTopic> = Vec::new();
        for topic in topics {
            computed_topics.push(topic.into());
        }
        ...
        Box::new({
            move |node_id| {
                ...
                let topics = if node_id < da_committee_size as u64 {
                    vec![Topic::DA.into(), Topic::Global.into()]
                } else {
                    vec![Topic::Global.into()]
                };
