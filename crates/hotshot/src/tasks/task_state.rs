    ...
    for NetworkRequestState<TYPES, I>
{
    async fn create_from(handle: &SystemContextHandle<TYPES, I, V>) -> Self {
        Self {
            ...
            temporal_state: handle.hotshot.temporal_state.reader(),
    ...
    for QuorumVoteTaskState<TYPES, I, V>
{
    async fn create_from(handle: &SystemContextHandle<TYPES, I, V>) -> Self {
        ...
        Self {
            ...
            temporal_state: handle.temporal_state.reader(),
            instance_state: handle.hotshot.instance_state(),
    ...
    for ConsensusTaskState<TYPES, I, V>
{
    async fn create_from(handle: &SystemContextHandle<TYPES, I, V>) -> Self {
        ...
        Self {
            ...
            timeout: handle.hotshot.config.next_view_timeout,
            consensus: OuterConsensus::new(consensus),
            temporal_state: handle.hotshot.temporal_state.writer(),
            last_decided_view: handle.hotshot.temporal_state.cur_view(),
            id: handle.hotshot.id,
            upgrade_lock: handle.hotshot.upgrade_lock.clone(),
        }
    }
