...
>(
    ...
    let mut network_state: NetworkMessageTaskState<_> = NetworkMessageTaskState {
...
///
/// # Returns
/// A future that resolves when a `HotShotEvent::Shutdown` is detected.
///
/// # Usage
/// Use in `select!` macros or similar constructs for graceful shutdowns:
pub async fn shutdown_event<TYPES: NodeType>(mut event_stream: Receiver<Arc<HotShotEvent<TYPES>>>) {
    ...
    Self: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    async fn add_network_tasks(&'static mut self, handle: &mut SystemContextHandle<TYPES, I, V>) {
        ...
        let internal_event_stream = handle.internal_event_stream.1.activate_cloned();
                ...
                futures::select! {
                    ...
                    event = original_recv_event => {
                        match event {
                            ...
                            Err(e) => {
                                tracing::error!("Error in byzantine layer sender: {:?}", e);
                                return;
                            }
                        }
                    }
                ...
                futures::select! {
                    ...
                    event = network_recv_event => {
                        match event {
                            ...
                            Err(e) => {
                                tracing::error!("Error in byzantine layer receiver: {:?}", e);
                                return;
                            }
                        }
                    }
