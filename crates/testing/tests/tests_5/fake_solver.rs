...
async fn test_fake_solver_fetch_non_permissioned_no_error() {
    ...
    // Fire up the solver.
    let solver_url: Url = format!("http://localhost:{}", portpicker::pick_unused_port().unwrap()).parse().unwrap();
    ...
    // Then, hit the API
    let resp = client.get(solver_url.join("v0/api/auction_results/1").unwrap())
        .send()
        .await
        .unwrap()
...
async fn test_fake_solver_fetch_non_permissioned_with_errors() {
    ...
    for i in 0..10 {
        // Then, hit the API
        let resp = client.get(solver_url.join(&format!("v0/api/auction_results/{i}")).unwrap())
            .send()
            .await;
...
async fn test_fake_solver_fetch_permissioned_no_error() {
    ...
    let client = reqwest::Client::new();
    let encoded_signature: tagged_base64::TaggedBase64 = <TestTypes as NodeType>::SignatureKey::sign(&private_key, &[0;32]).unwrap().into();

    // Then, hit the API
    let resp = client.get(solver_url.join(&format!("v0/api/auction_results/1/{encoded_signature}")).unwrap())
        .send()
        .await
        .unwrap()
        .json::<Vec<HashMap<String, String>>>()
...
async fn test_fake_solver_fetch_permissioned_with_errors() {
    ...
    let client = reqwest::Client::new();
    let mut payloads = Vec::new();
    let encoded_signature: tagged_base64::TaggedBase64 = <TestTypes as NodeType>::SignatureKey::sign(
        &private_key, &[0;32]
    ).unwrap().into();
    ...
    for i in 0..10 {
        // Then, hit the API
        let resp = client
            .get(solver_url.join(&format!("v0/api/auction_results/{i}/{encoded_signature}")).unwrap())
            .send()
            .await;
