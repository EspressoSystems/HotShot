...
pub type NodeMap<TYPES> = BTreeMap<<TYPES as NodeType>::ViewTime, Vec<Leaf<TYPES>>>;

/// A sanitized map from views to leaves for a single node, with only a single leaf per view.
pub type NodeMapSanitized<TYPES> = BTreeMap<<TYPES as NodeType>::ViewTime, Leaf<TYPES>>;
...
async fn validate_node_map<TYPES: NodeType, V: Versions>(
    ...
    let mut view_decided = TYPES::ViewTime::new(0);
...
pub type ViewMap<TYPES> = BTreeMap<<TYPES as NodeType>::ViewTime, BTreeMap<usize, Leaf<TYPES>>>;
...
pub type ViewMapSanitized<TYPES> = BTreeMap<<TYPES as NodeType>::ViewTime, Leaf<TYPES>>;
