...
pub enum OverallSafetyTaskErr<TYPES: NodeType> {
    ...
    TooManyFailures(HashSet<TYPES::ViewTime>),

    #[error("Inconsistent failed views: expected: {expected_failed_views:?}, actual: {actual_failed_views:?}")]
    InconsistentFailedViews {
        expected_failed_views: Vec<TYPES::ViewTime>,
        actual_failed_views: HashSet<TYPES::ViewTime>,
    },
    #[error(
...
{
    async fn handle_view_failure(&mut self, num_failed_views: usize, view_number: TYPES::ViewTime) {
    ...
    async fn handle_event(&mut self, (message, id): (Self::Event, usize)) -> Result<()> {
        ...
        let key = match event {
            ...
            EventType::Decide {
                ...
                if leaf_chain.last().unwrap().leaf.view_number() == TYPES::ViewTime::genesis() {
                    return Ok(());
                }
...
pub struct RoundCtx<TYPES: NodeType> {
    ...
    pub round_results: HashMap<TYPES::ViewTime, RoundResult<TYPES>>,
    /// during the run view refactor
    pub failed_views: HashSet<TYPES::ViewTime>,
    /// successful views
    pub successful_views: HashSet<TYPES::ViewTime>,
}
...
pub struct OverallSafetyPropertiesDescription<TYPES: NodeType> {
    ...
    pub expected_views_to_fail: HashMap<TYPES::ViewTime, bool>,
}
