    ...
    for (stage_number, stage) in script.iter_mut().enumerate() {
        ...
        for input in &stage.inputs {
            if !task.state_mut().filter(&Arc::new(input.clone())) {
                ...
                // Broadcast on the channel to ensure that downstream receivers and background
                // tasks get access to events.
                to_task
        ...
        for assert in &stage.outputs {
            if let Ok(Ok(received_output)) =
                async_timeout(Duration::from_millis(250), from_task.recv_direct()).await
            {
