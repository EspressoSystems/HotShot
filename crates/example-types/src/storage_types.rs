...
pub struct TestStorageState<TYPES: NodeType> {
    ...
    das: HashMap<TYPES::ViewTime, Proposal<TYPES, DaProposal<TYPES>>>,
    proposals: BTreeMap<TYPES::ViewTime, Proposal<TYPES, QuorumProposal<TYPES>>>,
    high_qc: Option<hotshot_types::simple_certificate::QuorumCertificate<TYPES>>,
    action: TYPES::ViewTime,
}
...
impl<TYPES: NodeType> TestStorage<TYPES> {
    ...
    pub async fn last_actioned_view(&self) -> TYPES::ViewTime {
...
impl<TYPES: NodeType> Storage<TYPES> for TestStorage<TYPES> {
    ...
    async fn update_undecided_state(
        ...
        _state: BTreeMap<TYPES::ViewTime, View<TYPES>>,
    ) -> Result<()> {
        if self.should_return_err {
            bail!("Failed to update high qc to storage");
        }
