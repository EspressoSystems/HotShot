...
pub(crate) async fn get_parent_leaf_and_state<TYPES: NodeType>(
    ...
    let parent_view_number = task_state.high_qc.get_view_number();

    let parent_view = task_state.validated_states.get(&parent_view_number).context(
    ...
    let reached_decided = leaf.get_view_number() == task_state.last_decided_view;
    ...
    if !reached_decided {
        ...
        while let Some(next_parent_leaf) = task_state.undecided_leaves.get(&next_parent_hash) {
            if next_parent_leaf.get_view_number() <= task_state.last_decided_view {
                break;
            }
