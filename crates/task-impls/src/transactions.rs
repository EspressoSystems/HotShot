...
impl<TYPES: NodeType, I: NodeImplementation<TYPES>, V: Versions> TransactionTaskState<TYPES, I, V> {
    ...
    async fn last_vid_commitment_retry(
        ...
        block_view: TYPES::ViewTime,
        task_start_time: Instant,
    ) -> Result<(TYPES::ViewTime, VidCommitment)> {
        loop {
    ...
    async fn last_vid_commitment(
        ...
        let mut target_view = TYPES::ViewTime::new(block_view.saturating_sub(1));
        ...
        loop {
            ...
            match view_data.view_inner {
                ...
                ViewInner::Failed => {
                    ...
                    target_view = TYPES::ViewTime::new(
    ...
    async fn wait_for_block(&self, block_view: TYPES::ViewTime) -> Option<BuilderResponse<TYPES>> {
        let task_start_time = Instant::now();
