        ...
        match event.as_ref() {
            HotShotEvent::UpgradeProposalRecv(proposal, sender) => {
                ...
                // TODO Come back to this - we probably don't need this, but we should also never receive a UpgradeCertificate where this fails, investigate block ready so it doesn't make one for the genesis block
                    ...
                    self.cur_view != TYPES::Time::genesis() && *view >= self.cur_view.saturating_sub(1),
                    format!("Discarding old upgrade proposal; the proposal is for view {:?}, but the current view is {:?}.",
                      view,
                      self.cur_view
                    )
                );
                ...
                let view_leader_key = self.quorum_membership.leader(view).await?;
                ensure!(
            ...
            HotShotEvent::UpgradeVoteRecv(ref vote) => {
                ...
                // Check if we are the leader.
                {
                    let view = vote.view_number();
                        ...
                        self.quorum_membership.leader(view).await? == self.public_key,
                        format!(
                            "We are not the leader for view {} are we leader for next view? {}",
                            *view,
                            self.quorum_membership.leader(view + 1).await? == self.public_key
                        )
                    );
                }
