...
pub enum HotShotEvent<TYPES: NodeType> {
    ...
    QuorumVoteDependenciesValidated(TYPES::ViewTime),
    ...
    ViewSyncTrigger(TYPES::ViewTime),
    ...
    BlockReady(VidDisperse<TYPES>, TYPES::ViewTime),
    ...
    ValidatedStateUpdated(TYPES::ViewTime, View<TYPES>),

    /// A new locked view has been created (2-chain)
    LockedViewUpdated(TYPES::ViewTime),

    /// A new anchor view has been successfully reached by this node (3-chain).
    LastDecidedViewUpdated(TYPES::ViewTime),
...
impl<TYPES: NodeType> HotShotEvent<TYPES> {
    ...
    pub fn view_number(&self) -> Option<TYPES::ViewTime> {
        match self {
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            HotShotEvent::LeafDecided(leaves) => {
                let view_numbers: Vec<<TYPES as NodeType>::ViewTime> =
