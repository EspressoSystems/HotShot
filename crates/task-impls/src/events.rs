...
pub enum HotShotEvent<TYPES: NodeType> {
    ...
    QuorumProposalRecv(Proposal<TYPES, QuorumProposal<TYPES>>, TYPES::PublicKey),
    ...
    DAProposalRecv(Proposal<TYPES, DAProposal<TYPES>>, TYPES::PublicKey),
    ...
    QuorumProposalSend(Proposal<TYPES, QuorumProposal<TYPES>>, TYPES::PublicKey),
    /// Send a quorum vote to the next leader; emitted by a replica in the consensus task after seeing a valid quorum proposal
    QuorumVoteSend(QuorumVote<TYPES>),
    /// Send a DA proposal to the DA committee; emitted by the DA leader (which is the same node as the leader of view v + 1) in the DA task
    DAProposalSend(Proposal<TYPES, DAProposal<TYPES>>, TYPES::PublicKey),
    ...
    DACSend(DACertificate<TYPES>, TYPES::PublicKey),
    ...
    ViewSyncPreCommitCertificate2Send(ViewSyncPreCommitCertificate2<TYPES>, TYPES::PublicKey),
    /// Send a `ViewSyncCommitCertificate2` from the network; emitted by a relay in the view sync task
    ViewSyncCommitCertificate2Send(ViewSyncCommitCertificate2<TYPES>, TYPES::PublicKey),
    /// Send a `ViewSyncFinalizeCertificate2` from the network; emitted by a relay in the view sync task
    ViewSyncFinalizeCertificate2Send(ViewSyncFinalizeCertificate2<TYPES>, TYPES::PublicKey),
    ...
    TransactionSend(TYPES::Transaction, TYPES::PublicKey),
    ...
    VidDisperseSend(Proposal<TYPES, VidDisperse<TYPES>>, TYPES::PublicKey),
    /// Vid disperse data has been received from the network; handled by the DA task
    ///
    /// Like [`HotShotEvent::DAProposalRecv`].
    VidDisperseRecv(Proposal<TYPES, VidDisperse<TYPES>>, TYPES::PublicKey),
}
