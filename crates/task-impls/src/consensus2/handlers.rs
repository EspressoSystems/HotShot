...
>(
    ...
    if task_state.quorum_membership.get_leader(old_view_number) == task_state.public_key {
...
pub(crate) async fn handle_timeout<TYPES: NodeType, I: NodeImplementation<TYPES>, V: Versions>(
    ...
    task_state
        ...
        .number_of_timeouts
        .add(1);
    if task_state.quorum_membership.get_leader(view_number) == task_state.public_key {
        task_state
            .consensus
            .read()
            .await
