...
impl<TYPES: NodeType> ProposalDependencyHandle<TYPES> {
    pub async fn publish_proposal(
        ...
        let parent_view = consensus
            ...
            .context(
            format!("Couldn't find parent view in state map, waiting for replica to see proposal; parent view number: {}", **parent_view_number))?;

        // Leaf hash in view inner does not match high qc hash - Why?
        let (leaf_commitment, state) = parent_view
            .get_leaf_and_state()
            .context(format!(
        ...
        //     if view_is_between_versions(self.view_number, &upgrade_cert.data) {
        ...
        let signature = TYPES::SignatureKey::sign(&self.private_key, new_leaf.commit().as_ref())
            .context("Failed to sign new_leaf.commit()!")?;
        // let Ok(signature) =
...
impl<TYPES: NodeType, I: NodeImplementation<TYPES>> QuorumProposalTaskState<TYPES, I> {
    ...
    #[instrument(skip_all, fields(
        id = self.id,
        latest_proposed_view = *self.latest_proposed_view
    ), name = "Quorum proposal create dependency task", level = "error")]
    ...
    async fn update_view(
        ...
        bail!(
            "Provided view {new_view:?} is older than the current view {:?}",
            self.cur_view
        );
    }
    ...
    async fn handle_quorum_proposal_recv(
        &mut self,
        ...
        let parent = if justify_qc.is_genesis {
            ...
            {
                Some(leaf) => {
                    if let (Some(state), _) = consensus.get_state_and_delta(leaf.get_view_number())
                    {
        ...
        let Some((parent_leaf, parent_state)) = parent else {
            ...
            //         && high_qc.view_number == self.current_proposal.clone().unwrap().view_number;
